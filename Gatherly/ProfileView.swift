            ...
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        case "My Events":
                            Text("Your created events (\(eventsVM.events.count))")
                                .font(.headline)
                        case "RSVP'd":
                            Text("RSVP’d Events (coming soon)")
                                .font(.headline)
                        case "Past Events":
                            let past = eventsVM.events.filter { ($0.parsedDate ?? .distantFuture) < Date() }
